...
class LinearCRF(nn.Module):
    ...
    def calculate_all_scores(self, lstm_scores: torch.Tensor) -> torch.Tensor:
        ...
        scores = self.transition.view(1, 1, self.label_size, self.label_size) \
                     .expand(batch_size, seq_len, self.label_size, self.label_size) + \
                 lstm_scores.view(batch_size, seq_len, 1, self.label_size). \
                     expand(batch_size, seq_len, self.label_size, self.label_size)
        return scores
    ...
    def constrainted_viterbi_decode(self, all_scores: torch.Tensor, word_seq_lens: torch.Tensor,
                                    annotation_mask: torch.Tensor = None) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        for wordIdx in range(1, sentLength):
            ...
            scoresIdx = scoresRecord[:, wordIdx - 1, :] \
                        ...
                        scores[:, wordIdx, :, :]
            if annotation_mask is not None:
                scoresIdx += annotation_mask[:, wordIdx, :] \
        ...
        lastScores = torch.gather(scoresRecord, 1,
                                  word_seq_lens.view(batchSize, 1, 1).expand(batchSize, 1, self.label_size) - 1) \
